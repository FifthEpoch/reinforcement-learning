...
class Cell:
    ...
    def __init__(self, _y, _x):
        ...
        self.cnt = 0
        self.q = np.array([5.0, 5.0, 5.0, 5.0])
...
class Grid:
    ...
    def show_pi(self, _avg_t, _episode=0, _eps=0.1, _alpha=0.5, _gamma=1.0):
        ...
        norm = plt.Normalize(np.amin(cnts) - 100, np.amax(cnts))
        ...
        plt.suptitle(f"Windy Gridworld policy "
                     f"[t={rd(_avg_t)}, "
                     f"episode={_episode}, "
                     f"ε={_eps}, "
                     f"α={_alpha}, "
    ...
    def show_grid(self):
        ...
        for (i, j), cell in np.ndenumerate(self.grid):
            table.add_cell(
                i, j, 0.2, 0.2, text=str(i) + ', ' + str(j), loc='center'
            )
        ax.add_table(table)
        plt.show()
